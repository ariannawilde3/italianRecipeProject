...
const REGIONI_PATHS: Record<RegioneItaliana, { d: string; label: { x: number; y: number } }> = {
  "Valle d'Aosta": {
...
function getColore(count: number): string {
  if (count === 0) return "#e8e0d4";
  if (count <= 2) return "#f0c8a0";
  if (count <= 5) return "#e8a060";
  if (count <= 10) return "#d07030";
  return "#b84020";
}
...
export default function MappaItalia({ onRegioneClick, conteggio = {} }: MappaItaliaProps) {
  ...
  return (
    <div className="mappa-container">
      ...
      >
        {(Object.entries(REGIONI_PATHS) as [RegioneItaliana, typeof REGIONI_PATHS[RegioneItaliana]][]).map(
          ([regione, { d, label }]) => {
            ...
            return (
              <g key={regione}>
                <path
                  d={d}
                  fill={isHovered ? "#c9402d" : getColore(count)}
                  stroke="#5a3e2b"
      ...
      {regioneHover && (
        <div className="mappa-tooltip">
          ...
          <span>{conteggio[regioneHover] || 0} ricette</span>
        </div>
      )}
    </div>
  );
